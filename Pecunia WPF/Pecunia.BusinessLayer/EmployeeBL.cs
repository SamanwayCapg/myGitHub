...
namespace Capgemini.Pecunia.BusinessLayer
{
    ...
    public class EmployeeBL : BLbase<Employee>, IEmployeeBL, IDisposable
    {
        ...
        public async override Task<bool> Validate(Employee entityObject)
        {
        ...
        public async Task<Employee> GetEmployeeByEmployeeIDBL(Guid searchEmployeeID)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    matchingEmployee = employeeDAL2.GetEmployeeByEmployeeIDDAL(searchEmployeeID);
                });
            }
            catch (PecuniaException ex)
            {
        ...
        public async Task<Employee> GetEmployeeByEmailAndPasswordBL(string email, string password)
        {
            ...
            try
            {
                ...
                if (matchingEmployee == default(Employee))
        ...
        public async Task<bool> UpdateEmployeeBL(Employee updateEmployee)
        {
            ...
            try
            {
                bool isValid = await Validate(updateEmployee);
                if ((isValid == true) && (await GetEmployeeByEmployeeIDBL(updateEmployee.EmployeeID)) != null)
                {
        ...
        /// <returns>Determinates whether the existing Employee is deleted.</returns>
